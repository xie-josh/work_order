...
class FbAccountConsumptionTrusteeship
{
    public function fire(Job $job, $data)
    {
        //php think queue:listen --queue FbAccountConsumptionTrusteeship
        //sleep(1);
    ...
    {   
        try {
            ...
            DB::table('ba_accountrequest_proposal_trusteeship')->where('account_id',$accountId)->update(['pull_consumption'=>date('Y-m-d H:i',time())]);
            ...
            DB::table('ba_account_consumption_trusteeship')->where('account_id',$accountId)->whereIn('date_start',$sSTimeList)->delete();
            ...
            // $this->fbSpendCap($params);
            DB::table('ba_account_consumption_trusteeship')->insertAll($data);
            ...
            DB::table('ba_fb_logs')->insert(
                ['log_id'=>$accountId??'','type'=>'job_FbAccountConsumptionTrusteeship','data'=>json_encode($params),'logs'=>$logs,'create_time'=>date('Y-m-d H:i:s',time())]
            );
    ...
    function fbSpendCap($params)
    {
        try {
            ...
            DB::table('ba_accountrequest_proposal_trusteeship')->where('account_id', $accountrequestProposal['account_id'])->update(
                [
            ...
            DB::table('ba_fb_logs')->insert(
                ['log_id'=>$accountrequestProposal['account_id'],'type'=>'job_fb_spend_cap_trusteeship','data'=>json_encode($params),'logs'=>$logs,'create_time'=>date('Y-m-d H:i:s',time())]
            );
        }
        return true;
    }
