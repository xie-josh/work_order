 ...
 * 账户列管理
 */
class AccountrequestProposalTrusteeship extends Backend
{
    /**
     * AccountrequestProposal模型对象
     * @var object
     * @phpstan-var \app\admin\model\addaccountrequest\AccountrequestProposalTrusteeship
     */
    ...
    {
        parent::initialize();
        $this->model = new \app\admin\model\addaccountrequest\AccountrequestProposalTrusteeship();
    }
    ...
    public function adminIndex($status)
    {
        ...
        foreach($where as $k => &$v)
        {
            if($v[0] == 'channel.nickname'){
                ...
                array_push($where, ['ba_accountrequest_proposal_trusteeship.admin_id','in',$adminIds]);
                unset($where[$k]);
            }
...
//                $accountAmount = $v['money']??0;
    ...
    public function add(): void
    {
        if ($this->request->isPost()) {
            ...
            $affiliationBm = $this->request->param(false)['affiliation_bm'];
            $result = false;
            $this->model->startTrans();
            try {
                $ids = $data['ids'];
                $bm = $data['bm'];
                $trusteeship_user = $data['trusteeship_user'];
                $trusteeship_type = $data['trusteeship_type'];
                $affiliationBm = $data['affiliation_bm'];
                ...
                foreach($ids as $k =>$v){
                    ...
                    $cc = [
                        ...
                        'admin_id'=>$adminId,
                        'status'=>0,
                        'trusteeship_user'=>$trusteeship_user,
                        'trusteeship_type'=>$trusteeship_type,
                        // 'time_zone'=>$timeZone,
                        // 'currency'=>$currency,
                        'account_id'=>$v,
                        // 'is_cards'=>$isCards,
                    ...
                    // if($automaticCardIs == 2) $cc['cards_id'] = $cardList[$k];
                ...
                Db::table('ba_accountrequest_proposal_trusteeship')->insertAll($dataList);
                // if(!empty($cardList)) DB::table('ba_cards_info')->whereIn('cards_id',$cardList)->update(['is_use'=>1]);
    ...
    public function Export()
    {
        ...
        if($ids) array_push($where,['accountrequest_proposal_trusteeship.id','in',$ids]);
        ...
        ->field('accountrequest_proposal_trusteeship.*,account.name account_name,account.bm account_bm,admin.nickname,cards_info.card_no,account.status open_account_status')
        ->leftJoin('ba_account account','account.account_id=accountrequest_proposal_trusteeship.account_id')
        ->leftJoin('ba_admin admin','admin.id=accountrequest_proposal_trusteeship.admin_id')
        ->leftJoin('ba_cards_info cards_info','cards_info.cards_id=accountrequest_proposal_trusteeship.cards_id')
        ->order('accountrequest_proposal_trusteeship.id','asc')
        ->where($where);
        $total = $query->count(); 
