...
class AccountOpeningApplicationManage extends Backend
{
    ...
    public function addChannelRelatList()
    {
        ...
        $channelRelatList = DB::table('ba_aoam_channel_relat')->where('channel_id',$data['channel_id'])->where('status',1)->column('aoam_id');
        $statusCount = count($channelRelatList);
        foreach($list as &$v)
        {
            $v['status'] = in_array($v['id'],$channelRelatList) ? 1 : 2;
        }
